...
#pragma mark Workspaces
	 ...
	 *
	 * event		-- the CGSConnectionNotifyEvent that is triggering this callback
	 * unknowndata1	-- unknown data 1
	 * unknowndata2	-- unknown data 2
	 * context		-- pointer to the context passed to CGSRegisterConnectionNotifyProc
	 */
	
	typedef void (*CGConnectionNotifyProc)(CGSConnectionNotifyEvent event, CGSValue unknowndata1, CGSValue unknowndata2, void* context);
	
	/* Register a callback function to receive notifications about when
	 the current Space is changing.
	 *
	 * cid			-- an active CGSConnection, usually _CGSDefaultConnection()
	 * function		-- pointer to the intended callback function (must be in C; cannot be an Objective-C selector)
	 * event		-- a single CGSConnectionNotifyEvent (ie. CGSWorkspaceChangedEvent)
	 * context		-- pointer to user-defined auxiliary information structure; passed directly to callback proc
	 */
	
	// For spaces notifications, you would register the following:
	//	CGSRegisterConnectionNotifyProc( _CGSDefaultConnection(), mySpaceChangedCFunction, CGSWorkspaceChangedEvent, (void *)context );
	
	extern CGError CGSRegisterConnectionNotifyProc(const CGSConnection cid, CGConnectionNotifyProc function, CGSConnectionNotifyEvent event, void* context);
	extern CGError CGSRemoveConnectionNotifyProc(const CGSConnection cid, CGConnectionNotifyProc function, CGSConnectionNotifyEvent event, void* context);
